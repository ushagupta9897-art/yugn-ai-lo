...
import type { ContentGenerationParams, ChatMessage as ChatMessageType, AssetFile } from '../types';
...
const templates = [
  {
    category: 'Ads',
    items: [
      { name: 'Ad Copy (Pain-Agitate-Solution)', description: 'Highlight a problem and present your product as the relief.' },
      { name: 'Ad Copy (AIDA Model)', description: 'Attention, Interest, Desire, Action. A classic for a reason.' },
    ],
  },
  {
    category: 'Social Media',
    items: [
      { name: 'Social Media Post (Feature to Benefit)', description: 'Turn a product feature into a compelling user benefit.' },
      { name: 'Persona-Targeted Post', description: 'Speak directly to a specific customer persona.' },
    ],
  },
  {
    category: 'Blog Content',
    items: [
      { name: 'Blog Post Intro (Problem/Promise)', description: 'Hook readers by stating a problem and promising a solution.' },
    ],
  },
  {
    category: 'SEO',
    items: [
      { name: 'SEO-Optimized Answer', description: 'Directly answer a common question for SERP visibility.' },
    ],
  },
];
...
const ContentGenerator: React.FC = () => {
    ...
    const [params, setParams] = useState<ContentGenerationParams>({
        platform: 'linkedin',
        type: 'Ad Copy (Pain-Agitate-Solution)',
        topic: '',
        tone: 'professional',
        keywords: '',
        assetId: '',
    ...
    const handleGenerate = async () => {
        ...
        if (needsPersona && !params.personaName) {
            addToast("Please select a target persona for this template.", 'error');
            return;
        }
        setLoading(true);
        setChatHistory([]);
    ...
    const needsPersona = ['Ad Copy (Pain-Agitate-Solution)', 'Ad Copy (AIDA Model)', 'Persona-Targeted Post'].includes(params.type);
                ...
                >
                    <div className="p-6 border-b border-border-light dark:border-border-dark flex justify-between items-center">
                     ...
                     <div className="flex-1 p-6 space-y-5 overflow-y-auto">
                        <div>
                            ...
                            <div className="space-y-4 mt-2">
                                ...
                                <div key={group.category}>
                                    <h4 className="text-xs font-bold uppercase text-subtle-text-light dark:text-subtle-text-dark tracking-wider mb-2">{group.category}</h4>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    {group.items.map(template => (
                                        <button
                                        key={template.name}
                                        onClick={() => handleChange('type', template.name)}
                                        className={`p-3 text-left rounded-lg border-2 transition-all ${params.type === template.name ? 'border-primary bg-primary/10' : 'border-border-light dark:border-border-dark hover:border-primary/50'}`}
                                        >
                                        <p className="font-semibold text-sm">{template.name}</p>
                                        <p className="text-xs text-secondary-text-light dark:text-secondary-text-dark mt-1">{template.description}</p>
                                        </button>
                                    ))}
                                    </div>
                                </div>
                        ...
                        {needsPersona && strategy?.targetPersonas && (
