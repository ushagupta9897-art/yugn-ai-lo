...
import type { ResonanceReport, AdCreativeVariation, PersonaFeedback } from '../types';
...
const CreativeCard: React.FC<{
...
}> = ({ creative, feedback, isWinner, onGenerateVariations, onSendToContent, onSendToPlanner }) => (
    <Card className={`flex flex-col ${isWinner ? 'border-2 border-emerald-500 shadow-lg shadow-emerald-500/10' : ''}`}>
        ...
        <div className="mt-4 pt-4 border-t border-border-light dark:border-border-dark space-y-3">
            ...
            {feedback.map(f => (
                <div key={f.personaName} className="p-3 bg-slate-50 dark:bg-surface-dark/60 rounded-lg">
                    ...
                    {f.groundingFact && (
                        <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-700 flex items-start gap-2">
                            <DatabaseIcon className="w-4 h-4 text-slate-400 flex-shrink-0 mt-0.5" />
                            <p className="text-xs text-slate-500 dark:text-slate-400">
                                <span className="font-bold">Evidence Log:</span> {f.groundingFact}
                                {f.groundingUrl && (
                                    <a href={f.groundingUrl} target="_blank" rel="noopener noreferrer" className="ml-2 text-primary hover:underline font-semibold">(Source)</a>
                                )}
                            </p>
                        </div>
                    )}
        ...
        <div className="mt-auto pt-4 border-t border-border-light dark:border-border-dark space-y-3">
            <div className="space-y-2">
                <div className="flex flex-col sm:flex-row gap-2">
                 ...
                 <button onClick={onSendToPlanner} className="w-full flex items-center justify-center gap-2 text-sm font-semibold py-2 px-3 rounded-lg border-2 border-purple-500 text-purple-500 hover:bg-purple-500/10 transition-colors">
                    <PlannerIcon className="w-4 h-4" /> Generate Plan from Creative
                </button>
            </div>
            {creative.variations && creative.variations.length > 0 && (
                 <div className="p-3 bg-slate-50 dark:bg-surface-dark/60 rounded-lg animate-fade-in mt-3">
...
const ResonanceLab: React.FC = () => {
    const { getActiveProject, updateActiveProjectData, setActiveTab, setContentGeneratorPrefill, setPlannerPrefill } = useAppContext();
    ...
    const handleSendToContent = (creative: AdCreativeVariation) => {
        ...
        addToast("Content Generator pre-filled with creative!", 'info');
    };

    const handleSendToPlanner = (creative: AdCreativeVariation) => {
        setPlannerPrefill({
            topic: creative.headline,
            action: 'generate',
        });
        setActiveTab(Tab.Planner);
        addToast("Generating a new content plan from your selected creative...", 'info');
    };
                    ...
                    <div className="grid lg:grid-cols-2 xl:grid-cols-3 gap-6">
                        {report.generatedCreatives.map(creative => (
                            <CreativeCard 
                                ...
                                onSendToPlanner={() => handleSendToPlanner(creative)}
                            />
                        ))}
                    </div>
                </div>
