...
import type { MarketingAnalysis, ChatMessage as ChatMessageType, BudgetAnalysis, Task, PlatformStrategy, Persona, DiscoveredProduct, WebsiteAnalysisData, BusinessData, DiscoveredLinks, BudgetStrategy } from '../types';
import { generatePersonas, generatePlatformsAndBudget, generateRemainingStrategy, analyzeBudgetAppropriateness, generateMarketingStrategy, explainPlatformChoice, discoverProductsFromWebsiteUrl, summarizeDiscoveredProduct, analyzeWebsiteForAnalysisData, classifyChatIntent } from '../services/geminiService';
...
import { StrategyIcon } from '../components/icons/TabIcons';
...
const StrategyBuilder: React.FC = () => {
    ...
    const [generationStage, setGenerationStage] = useState<'idle' | 'generatingPersonas' | 'awaitingPersonaApproval' | 'generatingPlatforms' | 'awaitingPlatformApproval' | 'generatingFinal'>('idle');
    ...
    }, [businessData?.budget, businessData?.industry, businessData?.businessGoals, businessData?.geography, businessData?.companySize]);
    ...
    const handleStartGeneration = async () => {
        ...
        setTasks([{ name: "Generating Target Personas...", status: 'running' }]);
        setGenerationStage('generatingPersonas');

        try {
            const personas = await generatePersonas(businessData, activeProject.knowledgeBase);
            updateActiveProjectData({ strategy: { targetPersonas: personas } as MarketingAnalysis });
            setTasks(prev => prev.map(t => ({...t, status: 'complete'})));
            setGenerationStage('awaitingPersonaApproval');
        } catch (err) {
            addToast(err instanceof Error ? err.message : "An unknown error occurred.", 'error');
            setAriaLiveMessage('An error occurred while generating personas.');
    ...
    const handleApprovePersonas = async () => {
        const currentProject = getActiveProject();
        const currentStrategy = currentProject?.strategy;
        if (!businessData || !currentStrategy?.targetPersonas) return;

        setTasks([
            { name: "Generating Target Personas", status: 'complete' },
            { name: "Generating Platforms & Budget...", status: 'running' },
        ]);
        setGenerationStage('generatingPlatforms');
        try {
            const { platformRecommendations, budgetStrategy } = await generatePlatformsAndBudget(businessData, currentStrategy.targetPersonas, currentProject?.knowledgeBase);
            updateActiveProjectData({ strategy: { ...currentStrategy, platformRecommendations, budgetStrategy } });
            setTasks(prev => prev.map(t => ({...t, status: 'complete'})));
            setGenerationStage('awaitingPlatformApproval');
        } catch (err) {
            addToast(err instanceof Error ? err.message : "An unknown error occurred.", 'error');
            setAriaLiveMessage('An error occurred while generating platforms.');
            setGenerationStage('awaitingPersonaApproval'); // Go back to previous step on error
        }
    };
    ...
    const handleApproveAndFinalize = async () => {
        const currentProject = getActiveProject();
        const currentStrategy = currentProject?.strategy;
        if (!businessData || !currentStrategy?.targetPersonas || !currentStrategy?.platformRecommendations) return;
        
        setTasks([
             { name: "Generating Target Personas", status: 'complete' },
            { name: "Generating Platforms & Budget", status: 'complete' },
            { name: "Synthesizing Final Strategy...", status: 'running' },
        ]);
        setGenerationStage('generatingFinal');
        try {
            const remainingStrategy = await generateRemainingStrategy(businessData, currentStrategy.targetPersonas, currentStrategy.platformRecommendations, currentProject?.knowledgeBase);
            updateActiveProjectData({ strategy: { ...currentStrategy, ...remainingStrategy } });
            setTasks(prev => prev.map(t => ({...t, status: 'complete'})));
            setAriaLiveMessage('Niti AI marketing strategy has been successfully generated.');
            const initialAiMessage: ChatMessageType = { sender: 'ai', text: "Here is the complete marketing strategy. You can ask me to refine it or ask any questions using the chat below." };
            setChatHistory([initialAiMessage]);
            setGenerationStage('idle'); // Resets to show full results view
        } catch (err) {
            addToast(err instanceof Error ? err.message : "An unknown error occurred.", 'error');
            setAriaLiveMessage('An error occurred while finalizing the strategy.');
            setGenerationStage('awaitingPlatformApproval');
        }
    }

    const handleSendMessage = async () => {
        if (!chatMessage.trim() || isChatLoading || !businessData || !results) return;
        ...
        const currentProject = getActiveProject();
        const knowledgeBase = currentProject?.knowledgeBase;

        try {
            const intent = await classifyChatIntent(userText);
            let newAiMessage: ChatMessageType;

            if (intent === 'question') {
                const context = JSON.stringify(results, null, 2);
                const response = await generateGroundedContent(userText, `The user is asking a question in the context of this marketing strategy:\n${context}`);
                newAiMessage = { sender: 'ai', text: response.text, sources: response.sources };
            } else { // intent === 'feedback'
                const refinedStrategy = await generateMarketingStrategy(businessData, results, userText, knowledgeBase);
                updateActiveProjectData({ strategy: refinedStrategy });
                newAiMessage = { sender: 'ai', text: "I've updated the strategy based on your feedback. Take a look at the changes above. How else can I help?" };
            }

            setChatHistory(prev => [...prev.slice(0, -1), newAiMessage]);

            // Update knowledge base
            const conversationSnippet = `\n\n---\n[User Query]: ${userText}\n[AI Response]: ${newAiMessage.text}\n---`;
            updateActiveProjectData({ knowledgeBase: (knowledgeBase || '') + conversationSnippet });
            addToast("Learnings added to Project Brain.", 'info');

        } catch (e) {
            console.error("Failed to handle chat message:", e);
            const errorMessage = e instanceof Error ? e.message : "Failed to get a response from Niti AI.";
            addToast(errorMessage, 'error');
            setChatHistory(prev => prev.slice(0, -1)); // Remove loading indicator
        } finally {
    ...
    const isLoading = generationStage !== 'idle' && generationStage !== 'awaitingPersonaApproval' && generationStage !== 'awaitingPlatformApproval';

    const renderInteractiveGeneration = () => {
        if (isLoading) {
             return (
        ...
        if (generationStage === 'awaitingPersonaApproval') {
            const personas = activeProject?.strategy?.targetPersonas || [];
            return (
                 <div className="animate-fade-in-up space-y-8">
                    <h2 className="text-3xl font-bold font-heading text-center">Step 1: Validate Your Target Personas</h2>
                    <p className="text-center text-secondary-text-light dark:text-secondary-text-dark max-w-2xl mx-auto">
                        Niti AI has identified the following customer personas based on your business data and web research. Please review them. If they look correct, approve them to continue building your strategy.
                    </p>
                    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                        {personas.map(p => <PersonaCard key={p.name} persona={p} />)}
                    </div>
                    <div className="flex justify-center gap-4 pt-6 border-t border-border-light dark:border-border-dark">
                         <button onClick={() => { updateActiveProjectData({ strategy: null }); setGenerationStage('idle'); }} className="px-6 py-3 text-sm font-semibold rounded-lg bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 transition-colors">
                            Start Over
                        </button>
                        <button onClick={handleApprovePersonas} className="px-8 py-3 text-sm text-white font-semibold rounded-lg bg-primary hover:bg-primary-hover transition-all shadow-md shadow-primary/30">
                            Approve Personas & Continue →
                        </button>
                    </div>
                 </div>
            );
        ...
        if (generationStage === 'awaitingPlatformApproval') {
            const platforms = activeProject?.strategy?.platformRecommendations || [];
            const budget = activeProject?.strategy?.budgetStrategy;
             return (
                 <div className="animate-fade-in-up space-y-8">
                    <h2 className="text-3xl font-bold font-heading text-center">Step 2: Validate Platforms & Budget</h2>
                     <p className="text-center text-secondary-text-light dark:text-secondary-text-dark max-w-2xl mx-auto">
                        Based on the personas you approved, here are the recommended platforms and budget split.
                    </p>
                    <div className="grid md:grid-cols-2 gap-6">
                        {platforms.map(p => <PlatformCard key={p.platformName} platform={p} onExplain={() => handleExplainPlatform(p.platformName)} />)}
                    </div>
                    {budget && (
                        <Card>
                            <h3 className="text-xl font-bold font-heading mb-4">💰 Budget Strategy</h3>
                            <p className="text-sm mb-4 text-secondary-text-light dark:text-secondary-text-dark">{budget.summary}</p>
                            <BudgetSplitBar splits={budget.platformSplits || []} />
                            <div className="flex justify-between mt-2 text-xs font-semibold text-secondary-text-light dark:text-secondary-text-dark">
                                {budget.platformSplits.map(s => <span key={s.platformName}>{s.platformName} ({s.percentage}%)</span>)}
                            </div>
                        </Card>
                    )}
                     <div className="flex justify-center gap-4 pt-6 border-t border-border-light dark:border-border-dark">
                         <button onClick={() => setGenerationStage('awaitingPersonaApproval')} className="px-6 py-3 text-sm font-semibold rounded-lg bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 transition-colors">
                            Back to Personas
                        </button>
                        <button onClick={handleApproveAndFinalize} className="px-8 py-3 text-sm text-white font-semibold rounded-lg bg-primary hover:bg-primary-hover transition-all shadow-md shadow-primary/30">
                            Approve & Generate Final Strategy ✨
                        </button>
                    </div>
                 </div>
            );
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
                ...
                <div className="max-w-7xl mx-auto">
                    {generationStage !== 'idle' ? renderInteractiveGeneration() :
                     results ? (
                        <div id="printable-strategy" className="animate-fade-in-up space-y-8">
                            ...
                            <Card className="mt-12 no-print">
                                ...
                                <p className="text-secondary-text-light dark:text-secondary-text-dark mb-4 text-base flex-shrink-0">Ask questions or provide feedback to iterate on this strategy.</p>
                                ...
                                <div className="relative border-t border-border-light dark:border-border-dark pt-4">
                                    <Input
                                        ...
                                        placeholder="e.g., 'Add a recommendation for influencer marketing' or 'What is a good ROAS?'"
                                    />
